...
class Face:
    ...
    def __init__(self):
        ...
        # Load Eyes
        utils.load_files("faces/eyes", self.special_attr_eyes,
                         self.eyes, "faces/eyes/")

        # Load Mouths
        utils.load_files("faces/mouths", self.special_attr_mouths,
                         self.mouths, "faces/mouths/")

        for i, value in self.mouths.items():
            self.hold_frames[i] = value[self.hold_overrides[i]
                                        ] if self.hold_overrides.get(i, None) else value[-1]
